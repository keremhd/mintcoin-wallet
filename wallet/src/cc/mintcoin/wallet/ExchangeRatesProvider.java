...
import cc.mintcoin.wallet.util.Io;
...
public class ExchangeRatesProvider extends ContentProvider
{
    ...
    private float mintBtcConversion = -1;
    ...
    private static final URL MINTPOOL_URL;
	...
	static
	{
		try
		{
			BITCOINAVERAGE_URL = new URL("https://api.bitcoinaverage.com/ticker/global/MNT");
            BLOCKCHAININFO_URL = new URL("http://mintcoin-explorer.info/");
            MINTPOOL_URL = new URL("http://mint.majorminers.com/");
            CRYPTSY_URL = new URL("http://pubapi.cryptsy.com/api.php?method=singlemarketdata&marketid=156");
            VIRCUREX_URL = new URL("https://vircurex.com/api/get_last_trade.json?base=MINT&alt=BTC");
		}
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs, final String sortOrder)
	{
		...
		if (lastUpdated == 0 || now - lastUpdated > UPDATE_FREQ_MS)
		{
            ...
            if ((mintBtcConversion == -1 && newMintBtcConversion == -1) || forceRefresh)
                newMintBtcConversion = requestMintBtcConversion(provider);
			...
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BITCOINAVERAGE_URL, mintBtcConversion, userAgent, BITCOINAVERAGE_FIELDS);
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BLOCKCHAININFO_URL, mintBtcConversion, userAgent, BLOCKCHAININFO_FIELDS);
			...
			if (newExchangeRates != null)
			{
                ...
                float mBTCRate = mintBtcConversion*1000;
		...
		if (exchangeRates == null || mintBtcConversion == -1)
			return null;
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final URL url, float mintBtcConversion, final String userAgent, final String... fields)
	{
						...
						for (final String field : fields)
						{
							...
							if (rate != null)
							{
								try
								{
									...
									if (mintRate.signum() > 0)
									{
										rates.put(currencyCode, new ExchangeRate(currencyCode, mintRate, url.getHost()));
										break;
									}
								}
    ...
    private static float requestMintBtcConversion(int provider) {
        HttpURLConnection connection = null;
        Reader reader = null;
        URL providerUrl;
        switch (provider) {
            case 0:
                providerUrl = MINTPOOL_URL;
                break;
            case 1:
