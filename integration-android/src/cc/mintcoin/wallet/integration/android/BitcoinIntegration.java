...
package cc.mintcoin.wallet.integration.android;
...
public final class BitcoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/mintcoin-paymentrequest"; // BIP 71
	...
	{
		final StringBuilder uri = new StringBuilder("mintcoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Mintcoin application found.\nPlease install Mintcoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=cc.mintcoin.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://langerhans.github.io/mintcoin-wallet-new/"));
